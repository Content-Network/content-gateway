...
export class YouTubeLoader extends HTTPDataLoaderBase<
    YoutubePlaylistResponse,
    PlaylistItem
> {
    // * this is the maximum that's allowed by the API
    protected batchSize = 50;
    protected type = PlaylistItem;
    ...
    private youtubePlayListId: string;
    ...
    constructor(
        ...
        super();
        this.info = schema; // todo should go into super
        // todo should register schema
    ...
    protected getUrlFor({ cursor }: LoadContext) {
        ...
        return `https://www.googleapis.com/youtube/v3/playlistItems?playlistId=${this.youtubePlayListId}&key=${this.youtubeApiKey}&maxResults=50&part=contentDetails,id,snippet,status${pagination}`;
    }

    protected mapResult(result: YoutubePlaylistResponse): Array<PlaylistItem> {
        return result.items
            .map((item) => {
                try {
                    return {
...
export const createYouTubeLoader = (
    youtubeApiKey: string,
    youtubePlayListId: string,
    schema: SchemaInfo
) => new YouTubeLoader(youtubeApiKey, youtubePlayListId, schema);
